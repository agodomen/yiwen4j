...
public class HBaseVODao implements VoInter {
		...
		try {
			...
			if (!result.isEmpty()) {
				for (byte[] bs : familyNames) {
					...
					// 获取此列族下的内容
					NavigableMap<byte[], byte[]> columnsMap = result.getFamilyMap(bs);
					// 获取此列族下的列名集合
					Set<byte[]> columnNameSet = columnsMap.keySet();
					// 将查出来的数据复制到HashMap中
					for (byte[] columnName : columnNameSet) {
						columnHashMap.put(Bytes.toString(columnName), Bytes.toString(columnsMap.get(columnName)));
					}
	...
	public boolean saveVO(HBaseVO aVo) {
		...
		for (String familyNameString : familyNameSet) {
			...
			for (String columnNameString : columnSet) {
				...
				put.add(Bytes.toBytes(familyNameString), Bytes.toBytes(columnNameString),
	...
	public List<String> getAllRowKeysForTable(String tableName) {
		...
		// TODO Auto-generated method stub
		if (null == tableName || tableName.equals(null) || tableName.length() <= 0) {
			System.out.println("表名不能为空！");
			return null;
		}
		List<String> rowKeyList = new ArrayList<String>();
		try {
			table = new HTable(HBaseCommon.getConfiguration(), tableName.getBytes());
		} catch (IOException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
	 ...
	 */
	public byte[] getValueOfColumn(String tableName, String rowKey, String familyName, String columnName) {
		// TODO Auto-generated method stub
		HBaseVO vo = new HBaseVO();
		vo.setRowKey(rowKey);
		byte[] re = null;
		try {
			...
			Get g = new Get(Bytes.toBytes(rowKey));
			Result result = table.get(g);
			List<KeyValue> columnKeyValues = result.getColumn(Bytes.toBytes(familyName), Bytes.toBytes(columnName));
	 ...
	 */
	public List<HBaseVO> getVosWhichRowKeyBetween(String tableName, String startRowKey, String endRowKey) {
		// TODO Auto-generated method stub
	...
	public static void main(String[] args) {
		...
		List<HBaseVO> list = dao.getVosWhichRowKeyBetween("Architecture", "a", "g");
		...
		System.out.println(Bytes.toString(dao.getValueOfColumn("Architecture", "jxd", "info", "race")));
